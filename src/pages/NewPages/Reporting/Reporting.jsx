...
function Reporting() {
  ...
  const getData = async (startDate, endDate, metricId) => {
    ...
    try {
      ...
      if (user.role == 2) {
        url = `${REACT_APP_API_URL}metrics/dashboard?companyId=${user.companyId}`;
        if (startDate && endDate) {
          url = `${REACT_APP_API_URL}metrics/dashboard?companyId=${user.companyId}&startDate=${startDate}&endDate=${endDate}`;
        }
        const params = {};
        ...
        url = `${REACT_APP_API_URL}metrics/dashboard`;
        if (startDate && endDate) {
