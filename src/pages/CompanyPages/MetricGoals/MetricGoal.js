...
const MetricGoal = () => {
  ...
  const getMetrics = async (value, status) => {
    try {
      let url = "";
      if (user.role == 1) {
        url = `${REACT_APP_API_URL}metrics?isDefault=true`;
      } else {
      ...
      <div className="custom-container mt-3">
        ...
        <div className="d-flex justify-content-between">
          <div className="mt-3 mb-3">
            ...
            >
              <span className="me-2">+</span> {user.role == 1 ? 'Add Default Metrics' : 'Add Metrics' }
            </button>
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              >
                <div className="modal-content">
                  ...
                  >
                    {isEditModal ? (
                      <h5 className="modal-title">{user.role == 1 ? 'Edit Default Metrics' : 'Edit Metrics' }</h5>
                    ) : (
                      <h5 className="modal-title">{user.role == 1 ? 'Add Default Metrics' : 'Add Metrics' }</h5>
                    )}
                      ...
                      <div>
                        <div className="mb-3">
                          ...
                          >
                            ...
                            {metrics.map((metric) => (
                              metric.type != "PERCENTAGE" ? <option value={metric.id}>{metric.name}</option> : ''
                            ))}
                          </select>
                        </div>
                        <div className="mb-3">
